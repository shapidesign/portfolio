...
export default function PlanetHero({ 
  title = "Cosmic Interfaces",
  subtitle = "Scroll to fly the camera through space. Realistic Earth textures, floating moon companion, and stunning post-processing effects.",
  customColors = {
    primary: '#66d9ef',
    secondary: '#a6e22e',
    accent: '#f92672'
  ...
  return (
    <div ref={containerRef} className="relative w-full h-[120vh] overflow-hidden rounded-2xl border-3 border-cyan-500">
      <Canvas camera={{ position: [0, 0, 12], fov: 45 }} dpr={[1, 2]}>
        <color attach="background" args={["#0c0c0c"]} />
        <Suspense fallback={
          <Html center>
            <span className="text-cyan-500 font-mono">Loading space...</span>
          </Html>
        }>
      ...
      <div className="pointer-events-none absolute inset-0 flex items-end">
        <div className="m-6 md:m-10 p-4 md:p-6 rounded-2xl bg-black/30 border border-cyan-500/20 backdrop-blur">
          <h1 className="text-white text-3xl md:text-5xl font-semibold tracking-tight font-mono">
            {title}
          </h1>
          <p className="text-white/80 mt-2 max-w-xl text-sm md:text-base font-mono">
            {subtitle}
          </p>
        </div>
      </div>
...
function RimAtmosphere({ radius = 1.05, color = '#88ccff', intensity = 1.0 }) {
  return (
    <mesh>
